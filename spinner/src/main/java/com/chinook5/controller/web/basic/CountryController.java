...
public class CountryController extends BaseController {
    ...
    public ResponseResult createCountry(Country country){
       // SystemUser systemUser = this.getSystemUser();
        try{
         //   country.setCreatedBy(systemUser.getSystemUserID().toString());
    ...
    public ResponseResult updateCountryDetail(@PathVariable int id , Country country) {
     //   SystemUser systemUser = this.getSystemUser();
        try{
         ...
         //   country.setModifyBy(systemUser.getSystemUserID().toString());
     ...
     * 查询国家列表(下拉框)
     */
    @RequestMapping(value = "getCountryForSpinner",method = RequestMethod.POST,produces = ResponseContentType.APPLICATION_JSON)
    @ResponseBody
    public List<PageData> getCountryForSpinner(){
        List<PageData> data = null;
        try {
            data = countryService.getCountryForSpinner();
        } catch (Exception e) {
           logger.error(e.toString());
        }
        return data;
